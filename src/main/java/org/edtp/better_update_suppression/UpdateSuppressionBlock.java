...
public class UpdateSuppressionBlock extends Block {
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        if (!world.isClient()) {
            ...
            world.setBlockState(pos, state.with(ACTIVED, my_state));
            if(my_state)
            {
            ...
            if (my_state) {
                ...
                for (PlayerEntity player : players) {
                    if (player.getBlockPos().isWithinDistance(pos, 16.0)) {
                        player.sendMessage(Text.literal("Update suppressed by ExampleBlock at: "+pos), false);
                    }
                }
